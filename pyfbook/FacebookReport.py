from pyfbook.MetaFacebookReport import MetaFacebookReport
import logging

from pyfbook.core.transform.path import get_config
...
def post_report(facebookreport, report, start, end):
...
def _post_reports(facebookreport, start, end):
    config = get_config(facebookreport)
    for report in config.get('reports'):
        post_report(facebookreport, report, start, end)
...
def get_reports_async(config, start=None, end=None, debug=False):
    ...
    """
    if not debug:
        _post_reports(config, start, end)
    bool_ = True
    while bool_:
...
class FacebookReport(MetaFacebookReport):
    def get_reports_async(self, start=None, end=None, debug=False):
        """
            config = config file
            start = YYYY-MM-DD
            end = YYYY-MM-DD
        ...
        """
        if not debug:
            _post_reports(self, start, end)
        bool_ = True
        while bool_:
            result = check_and_fetch_reports(self)
            if result:
                check_and_fetch_reports(self)
                bool_ = True
            else:
                bool_ = False
